...
using EsyaCekilisV3.Web.Areas.Admin.Controllers;
using Microsoft.AspNetCore.Authorization;
using Microsoft.EntityFrameworkCore.Storage;

namespace EsyaCekilisV3.Web.Areas.Admin
{
    ...
    public class RolesController : Controller
    {
        ...
        public async Task<IActionResult> RoleCreate(RoleCreateViewModel request) 
        {
            ...
            if(!result.Succeeded)
            {
                ModelState.AddModelErrorList((List<string>)result.Errors);
        ...
        {
            var currenUser = (await _userManager.FindByIdAsync(id))!; //güncel kullanıcıyı al   
            ViewBag.userId = id;
            var roles= await _roleManager.Roles.ToListAsync(); //normal rol bilgilerini al
